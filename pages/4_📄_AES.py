    ...
    if data and key and len(key) in [16, 24, 32]:
        ...
        if mode == "ECB":
            ...
            if operation == "Enkripsi":
                ...
                result = iv + cipher.encrypt(pad(data, AES.block_size))  # Simpan IV di awal
            else:
                iv = data[:16]  # Ambil IV dari ciphertext
                cipher = AES.new(key, AES.MODE_CBC, iv)
                result = unpad(cipher.decrypt(data[16:]), AES.block_size)

        elif mode == "CTR":
            if operation == "Enkripsi":
                nonce = get_random_bytes(8)  # 8 byte untuk nonce
                counter = Counter.new(64, prefix=nonce)
                cipher = AES.new(key, AES.MODE_CTR, counter=counter)
                result = nonce + cipher.encrypt(data)
            else:
                nonce = data[:8]  # Ambil nonce dari ciphertext
                counter = Counter.new(64, prefix=nonce)
                cipher = AES.new(key, AES.MODE_CTR, counter=counter)
                result = cipher.decrypt(data[8:])
